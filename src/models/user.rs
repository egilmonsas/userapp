...
use rocket_db_pools::sqlx::{Acquire, PgConnection};
...
<div><span class="label">Updated At: </span>{updated_at}</div>"#,
    ...
    async fn find_all_with_pagination(
        ...
        let query_str = "SELECT * FROM users WHERE created_at < $1 
        ...
        if users.len() == DEFAULT_LIMIT {
            let query_str = "SELECT EXISTS(SELECT 1 FROM users WHERE created_at < $1 
